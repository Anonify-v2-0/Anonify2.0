...
export async function discoverCloudModels(
  ...
  if (provider === "amazon-bedrock") {
    ...
    try {
      ...
      const models: ModelDefinition[] = (result.modelSummaries ?? [])
        .map((row) => ({
          id: row.modelId!,
          label: row.modelId!,
          textOutput: row.outputModalities?.includes("TEXT"),
          vision: row.inputModalities?.includes("IMAGE"),
          unavailable:
            row.modelLifecycle?.status === "LEGACY"
              ? "Legacy model"
              : !row.inferenceTypesSupported?.includes("ON_DEMAND")
                ? "Requires provisioned throughput or an inference profile"
                : undefined,
        }))
      ...
      do {
        ...
        for (const row of page.inferenceProfileSummaries ?? []) {
          if (row.inferenceProfileId)
            models.push({
              id: row.inferenceProfileId,
              label: row.inferenceProfileName || row.inferenceProfileId,
              unavailable:
                row.status !== "ACTIVE"
                  ? "Inactive inference profile"
                  : undefined,
            })
        }
      ...
      throw new DiscoveryError(
        "Bedrock discovery failed. Check AWS region, credentials, ListFoundationModels and ListInferenceProfiles permissions; a runtime-only key can still verify a typed model ID."
      )
    } finally {
      client.destroy()
    }
  ...
  if (provider === "azure") {
    if (
      ...
      throw new DiscoveryError(
        "Deployment discovery needs AZURE_SUBSCRIPTION_ID, AZURE_RESOURCE_GROUP and AZURE_RESOURCE_NAME. A deployment ID can also be entered and verified directly."
      )
    }
    ...
    try {
      ...
      throw new DiscoveryError(
        "Azure discovery needs an Azure CLI login, managed identity or service principal with permission to list deployments."
      )
    }
    ...
    try {
      ...
      throw new DiscoveryError(
        "Vertex model discovery needs Google Application Default Credentials with Model Garden access."
      )
    }
  ...
  while (next) {
    if (
      new URL(next).origin !== new URL(initial).origin ||
      seen.has(next) ||
      seen.size >= 100
    )
      throw new DiscoveryError("Invalid model-list pagination.")
    seen.add(next)
    const page = (await readModelJson(next, { headers }, fetcher)) as Record<
      string,
      unknown
    >
    ...
    if (!Array.isArray(rows))
      throw new DiscoveryError(
        "The cloud provider did not return a model list."
      )
    for (const row of rows) {
      ...
      if (
        ...
        model.unavailable = "Deployment is not ready"
      if (provider === "google-vertex" && !model.id.startsWith("gemini-"))
        model.unavailable =
          "This Vertex adapter uses Gemini generateContent models"
      models.push(model)
    }

...
Postgres and RustFS run in Docker, OCR runs through Tesseract, and no external
service is involved at any point.

The local stack moved because the MinIO server repository was archived and made
read-only in April 2026, after Community Edition binary releases had already
become source-only; the `mc` repository was archived in July 2026 as well.
RustFS is a better fit for this maintained development stack: it is actively
released under Apache-2.0, exposes the same standard S3 operations the app
already uses through the AWS SDK, publishes a maintained `rc` client for bucket
setup, and has a tested one-way import path for the existing object volume. See
[the migration guide](docs/migrate-object-storage-to-rustfs.md) for compatibility
...
already holds one of these ports, set `APP_PORT`, `POSTGRES_PORT`, `RUSTFS_PORT`
or `RUSTFS_CONSOLE_PORT` in `.env`; the addresses used inside the compose network
...
RustFS keeps object data in the existing `anonify_minio-data` Docker volume so
upgrading does not silently switch the app to an empty store. RustFS 1.0.0-rc.6
imports the unencrypted MinIO on-disk format used by Anonify in place. The move
is one-way after RustFS writes new metadata, so back up the volume before the
first start. Installations that enabled storage-server encryption, tiering, or
custom bucket features need the API-copy path described in
[Migrating object storage to RustFS](docs/migrate-object-storage-to-rustfs.md).
...
**AI detection is optional.** With the default Gateway and no `AI_GATEWAY_API_KEY`, pattern detection,
